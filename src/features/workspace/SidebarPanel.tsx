...
import { X, ChevronRight, ChevronDown, Folder, FileText, FolderOpen, Image, Music, Video, GripVertical, FolderPlus, FilePlus, SortAsc, ChevronsUpDown } from "lucide-react";
import { SETTINGS_GROUPS, SETTINGS_SECTIONS, type SettingsSectionId } from '@/features/settings/settingsNavigation';
                    ...
                    <CollapsibleContent className="pb-1 pl-3">
                      {sections.map((section) => (
                        <Button
                          ...
                          type="button"
                          variant="ghost"
                          onClick={() => selectSettingsSection(section.id, section.label)}
                          className={cn(
                            'h-8 w-full justify-start gap-2 px-2 text-xs font-normal',
                            activeSettingsSection === section.id && 'bg-sidebar-accent text-sidebar-accent-foreground'
                          )}
                        >
