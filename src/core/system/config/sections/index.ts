...
export function registerConfigSections(): void {
  ...
  registerConfigSection({
    ...
    write: (value) => {
      ...
      if (v.toolbar) {
        ...
        if (groups) {
          Object.entries(groups as Record<string, boolean>).forEach(
            ([g, enabled]) => s.updateToolbarGroup(g as never, enabled),
          );
        }
      }
    },
  ...
  registerConfigSection({
    ...
    read: () => {
      ...
      return {
        editorViewMode: s.editorViewMode,
  ...
  registerConfigSection({
    ...
    scope: "vault",
    read: () => ({
      layouts: (useWorkspaceStore.getState() as never as { layouts: unknown })
        .layouts,
    }),
    write: (value) => {
